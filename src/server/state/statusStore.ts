...
function emit(jobId: string, status: JobStatus) {
  listeners.get(jobId)?.forEach((cb) => {
    try {
      cb(status);
    } catch (e) {
...
export const statusStore = {
  async set(jobId: string, status: JobStatus, userId?: string) {
    const prevStoredStatus = statusMap.get(jobId)?.status;
    ...
    if (userId) {
      ...
      await db.collection("jobs").updateOne(
        ...
        {
          $setOnInsert: { jobId, userId: new ObjectId(userId), createdAt: now },
    ...
    const changed = prevStoredStatus !== snapshot.status;
    const shouldEmail =
      changed && SHOULD_EMAIL.includes(snapshot.status) && !!snapshot.userEmail;
